...
butler create-workflow \
	-n freebayes \
	-v 1.0 \
	-c $examples/workflows/freebayes-workflow/freebayes-workflow-config.json | sudo tee create-workflow.log

sudo -i \
butler create-analysis \
	-n freebayes \
	-d 01-01-2017 \
	-c $examples/analyses/freebayes-discovery/analysis.json | sudo tee create-analysis.log
